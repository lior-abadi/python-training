...
def search_command():
    lb1.delete(0, END)
    for row in backend.search(title_text.get(), autor_text.get(), year_text.get(), isbn_text.get()):
        lb1.insert(END, row)
...
def update_command():
    backend.update(selected_tuple[0], title_text.get(), autor_text.get(), year_text.get(), isbn_text.get())
    lb1.delete(0, END)
    refreshList()
